...
def results(list_numbers):
    #Validar que el parametros sea una lista no vacia
    if not isinstance(list_numbers, list) or len(list_numbers) == 0:
        raise ValueError ("El parametro debe ser una lista no vacia de numeros")
    
    #Validar que todos los elementos sea numericos (int o float)
    for num in list_numbers:
        if not isinstance(num,(int,float)):
            raise ValueError("La lista debe contener solo numeros enteros o decimales")
    
    #Calculo con numpy
    average = round(np.mean(list_numbers),2)
    std_dev = round(np.std(list_numbers),2)
