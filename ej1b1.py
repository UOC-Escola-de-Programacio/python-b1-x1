...
def obtain_max(list_numbers):
    #Validar que el parametro sea una lista no vacia
    if not isinstance(list_numbers, list) or len(list_numbers) == 0:
            raise ValueError ("El parametro debe ser una lista no vacia de numeros enteros.")
    
    #Validar que todos los elemntos sean enteros
    for num in list_numbers:
        if not isinstance (num, int):
             raise ValueError("La lista debe contener solo numeros enteros.")
        
    #Obetener el valor maximo
